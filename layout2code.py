...
def arg_parser():
    parser = argparse.ArgumentParser(description='Convert class definition in a l2c file to a fully fledged class in '
                                                 'a programming language')
    parser.add_argument('files', nargs='*', type=check_file, help='The list of files to compile to the language')
    parser.add_argument('--lang', nargs='?', default='java', choices=['java'], type=str, help='The language to compile '
                                                                                              'to, the default is java')
    return parser
...
def main():
    ...
    if len(files) == 0:
        # map the file names to the open_file func
